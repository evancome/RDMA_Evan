...
async def send_sq_meta(tb, wr_id, immdt, qpn, host, lst, offs, opcode, raddr, rlength, rkey, laddr, llength, lkey):
    tmp = 0
    # sq_meta_source
...
async def process_write(tb, wr_id, immdt, qpn, host, laddr, raddr, len):
    num_packets = (len + PMTU - 1) // PMTU
    # sq_meta
    # (tb, wr_id, qpn, host, lst, offs, opcode, raddr, rlength, rkey, laddr, llength, lkey)
    if (num_packets > 1):
        await send_sq_meta(tb, opcode=RC_RDMA_WRITE_FIRST, wr_id=wr_id, immdt=immdt, qpn=qpn, host=host, lst=0, offs=0, laddr=laddr, raddr=raddr, rlength=PMTU, llength=PMTU, rkey=0, lkey=0)
    else:
        if (immdt == 0):
            await send_sq_meta(tb, opcode=RC_RDMA_WRITE_ONLY, wr_id=wr_id, immdt=immdt, qpn=qpn, host=host, lst=1, offs=0, laddr=laddr, raddr=raddr, rlength=len, llength=len, rkey=0, lkey=0)
        else:
            await send_sq_meta(tb, opcode=RC_RDMA_WRITE_ONLY_WIT_IMD, wr_id=wr_id, immdt=immdt, qpn=qpn, host=host, lst=1, offs=0, laddr=laddr, raddr=raddr, rlength=len, llength=len, rkey=0, lkey=0)
    if (num_packets > 2):
        for i in range(1, num_packets-1):
            await send_sq_meta(tb, opcode=RC_RDMA_WRITE_MIDDLE, wr_id=wr_id, immdt=immdt, qpn=qpn, host=host, lst=0, offs=0, laddr=laddr+PMTU*i, raddr=raddr+PMTU*i, rlength=PMTU, llength=PMTU, rkey=0, lkey=0)
    if (num_packets > 1):
        if (immdt == 0):
            await send_sq_meta(tb, opcode=RC_RDMA_WRITE_LAST, wr_id=wr_id, immdt=immdt, qpn=qpn, host=host, lst=1, offs=0, laddr=laddr+PMTU*(num_packets-1), raddr=raddr+PMTU*(num_packets-1), rlength=((len+PMTU-1)%PMTU+1), llength=((len+PMTU-1)%PMTU+1), rkey=0, lkey=0)
        else:
            await send_sq_meta(tb, opcode=RC_RDMA_WRITE_LAST_WITH_IMD, wr_id=wr_id, immdt=immdt, qpn=qpn, host=host, lst=1, offs=0, laddr=laddr+PMTU*(num_packets-1), raddr=raddr+PMTU*(num_packets-1), rlength=((len+PMTU-1)%PMTU+1), llength=((len+PMTU-1)%PMTU+1), rkey=0, lkey=0)
...
async def process_read(tb, wr_id, qpn, host, laddr, raddr, len):
    ...
    if (num_page > 1):
        for i in range(num_page-1):
            await send_sq_meta(tb, opcode=RC_RDMA_READ_REQUEST, wr_id=wr_id, immdt=0, qpn=qpn, host=host, lst=0, offs=i, laddr=laddr, raddr=raddr, rlength=page, llength=page, rkey=0, lkey=0)
        await send_sq_meta(tb, opcode=RC_RDMA_READ_REQUEST, wr_id=wr_id, immdt=0, qpn=qpn, host=host, lst=1, offs=num_page-1, laddr=laddr, raddr=raddr, rlength=((len+page-1)%page+1), llength=((len+page-1)%page+1), rkey=0, lkey=0)
    else:
        await send_sq_meta(tb, opcode=RC_RDMA_READ_REQUEST, wr_id=wr_id, immdt=0, qpn=qpn, host=host, lst=1, offs=0, laddr=laddr, raddr=raddr, rlength=len, llength=len, rkey=0, lkey=0)
    # get read request
...
async def process_send(tb, wr_id, qpn, host, laddr, raddr, len):
    ...
    # sq_meta
    if (num_packets > 1):
        await send_sq_meta(tb, opcode=RC_SEND_FIRST, wr_id=wr_id, immdt=0, qpn=qpn, host=host, lst=0, offs=0, laddr=laddr, raddr=raddr, rlength=PMTU, llength=PMTU, rkey=0, lkey=0)
    else:
        await send_sq_meta(tb, opcode=RC_SEND_ONLY, wr_id=wr_id, immdt=0, qpn=qpn, host=host, lst=1, offs=0, laddr=laddr, raddr=raddr, rlength=len, llength=len, rkey=0, lkey=0)
    if (num_packets > 2):
        for i in range(1, num_packets-1):
            await send_sq_meta(tb, opcode=RC_SEND_MIDDLE, wr_id=wr_id, immdt=0, qpn=qpn, host=host, lst=0, offs=0, laddr=laddr+PMTU*i, raddr=raddr+PMTU*i, rlength=PMTU, llength=PMTU, rkey=0, lkey=0)
    if (num_packets > 1):
        await send_sq_meta(tb, opcode=RC_SEND_LAST, wr_id=wr_id, immdt=0, qpn=qpn, host=host, lst=1, offs=0, laddr=laddr+PMTU*(num_packets-1), raddr=raddr+PMTU*(num_packets-1), rlength=((len+PMTU-1)%PMTU+1), llength=((len+PMTU-1)%PMTU+1), rkey=0, lkey=0)
    # get mem data
    mem_read_cmds = await recv_mem_read_cmd(tb, num_packets)
    await send_rx_data_by_memcmd(tb, mem_read_cmds)
    await Timer(1000, 'ns')
...
async def run_test(dut, test_data=None, idle_inserter=None):
    ...
    # Write
    await send_sq_meta(tb, opcode=WR_RECEIVE, wr_id=1, immdt=0, qpn=5, host=0, lst=1, offs=0, laddr=0, raddr=0, rlength=0, llength=0, rkey=0, lkey=0)
    await process_write(tb, wr_id=2, immdt=0x999, qpn=2, host=0, laddr=0x2222222222222222, raddr=0x123456789abcdef, len=PMTU*5)
    await Timer(1000, 'ns')
    ...
    # # Write
    await send_sq_meta(tb, opcode=WR_RECEIVE, wr_id=2, immdt=0, qpn=5, host=0, lst=1, offs=0, laddr=0, raddr=0, rlength=0, llength=0, rkey=0, lkey=0)
    await process_write(tb, wr_id=4, immdt=0x888, qpn=2, host=0, laddr=0x4222222222222222, raddr=0x323456789abcdef, len=PMTU*8)
    # await Timer(1000, 'ns')
