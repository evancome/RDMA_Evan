...
async def run_test(dut, test_data=None, idle_inserter=None):
    ...
    # rx_bytes_read_mem_cmd = bytes(rx_frame_read_mem_cmd)
    # binary_read_mem_cmd = bin(int.from_bytes(rx_bytes_read_mem_cmd, byteorder='little'))[2:]
    # 
    # read_mem_addr  = int(binary_read_mem_cmd[48:],2)
    # read_mem_len   = int(binary_read_mem_cmd[0:48], 2)

    # read_mem  = int(binary_read_mem_cmd[0:47], 2)
    # print("收到内存读命令", hex(read_mem))
    # print("读内存地址：", hex(read_mem_addr))
    # print("读内存长度：", hex(read_mem_len))
...
def test_rocev2_top_write(request):
