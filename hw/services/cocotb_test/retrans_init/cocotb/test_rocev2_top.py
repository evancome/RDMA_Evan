...
# async def run_test(dut, tx_meta_length=1024, test_data=None, idle_inserter=None):
async def run_test(dut, test_data=None, idle_inserter=None):
    ...
    #     tb.log.info("        收到向外传输的数据包 ReadRequest:     %s", repr(rx_frame1))
    ...
    #     print("        收到ReadResponse数据帧 tx_frame1: %s", repr(tx_data_frame))
    ...
    # await tb.rx_data_source.send(tx_data_frames[0])
    # await tb.rx_data_source.send(tx_data_frames[1])
    # await tb.rx_data_source.send(tx_data_frames[3])
    # await tb.rx_data_source.send(tx_data_frames[2])
    # await tb.rx_data_source.send(tx_data_frames[4])
    # await tb.rx_data_source.send(tx_data_frames[5])
    ...
    # # 传ACK和NAK进RX
    # for pkt in rx_acks:
    #     await tb.rx_data_source.send(pkt)
    ...
    #     # tlast = tb.dut.m_axis_tx_data_tlast.value
    #     # tkeep = tb.dut.m_axis_tx_data_tkeep.value
    #     tb.log.info("        2收到向外传输的数据包 ReadRequest:     %s", repr(rx_frame1))
    ...
    # for i in range(num_mem_read_cmd):
    #     rx_frame_read_mem_cmd = await tb.mem_read_cmd_sink.recv()
    #     print("        2收到内存读命令 rx_frame1: %s", repr(rx_frame_read_mem_cmd))
    #     read_mem_cmd = MemCmd(rx_frame_read_mem_cmd.tdata)
    #     read_mem_cmd.print_MemCmd()
    #     read_mem_cmds.append(read_mem_cmd)
    ...
    #     tx_data_frame = await tb.tx_data_sink.recv()
    #     tx_data_frames.append(tx_data_frame)
    #     print("        2收到ReadResponse数据帧 tx_frame1: %s", repr(tx_data_frame))

    # await Timer(1000, 'ns')

    # # ReadResponse -> RX
    # for i in range(num_mem_read_cmd):
    #     print("2发送ReadResponse数据帧给RX: ", i)
    #     await tb.rx_data_source.send(tx_data_frames[i])
...
if cocotb.SIM_NAME:
    ...
    # factory.add_option("tx_meta_length", tx_meta_lengths)
